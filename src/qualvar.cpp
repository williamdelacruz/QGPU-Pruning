                   ...
                   float *temp1, int tsize, int xsize, int ysize)
{
  ...
  DxGradVar(temp1, result, xsize, ysize, tsize, bitflags, add_flag);
  printf("Extracting dy gradients\n");
  DyPhaseGradient(phase, temp1, xsize, ysize);
  add_flag = 1;
  printf("Extracting dy variances\n");
  /* add to dx variances */
  DyGradVar(temp1, result, xsize, ysize, tsize, bitflags, add_flag);
}
...
void DxGradVar(float *dx, float *dxvar, int xsize, int ysize,
  int tsize, unsigned char *bitflags, int add_code)
{
      ...
      for (i=0; i<xsize; i++) {
        ...
        for (n=0, b=j-hs; b<=j+hs; b++) {
          ...
          for (a=i-hs; a < i+hs; a++) {   /* a < to match dx */
            ...
            cc = bb*xsize + aa;
            if (bitflags && bitflags[cc]==BORDER) continue;
            r = dx[cc];
            avg += r;
            avgsqr += r*r;
            ++n;  
